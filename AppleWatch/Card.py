    ...
    cards1 = [
        ...
        dbc.Row([dbc.Col(dbc.Card(
                ...
                color="light",
                    ...
                    [
                        html.P("Exercise minutes Average|mean", className="card-text"),
...
def Card(df,df2, patient, group, date, value):
    ...
    if group == 'M':
        ...
        if len(str(value)) == 1:
            df2 = df2.loc[df2['month'] == '2020-0{}'.format(value)]
            df3 = df3.loc[df3['month'] == '2020-0{}'.format(value)]
            df4 = df4.loc[df4['month'] == '2020-0{}'.format(value)]
        else:
            df2 = df2.loc[df2['month'] == '2020-{}'.format(value)]
            df3 = df3.loc[df3['month'] == '2020-{}'.format(value)]
            df4 = df4.loc[df4['month'] == '2020-0{}'.format(value)]
        ...
        df4 = df.groupby(["@sourceName", 'DOW', '@type'])['@Value'].mean().reset_index()
        df = df.groupby(["@sourceName",'DOW', 'date', '@type'])['@Value'].sum().reset_index()
        df3 = df.groupby(["@sourceName",'DOW', '@type'])['@Value'].mean().reset_index()
        df2 = df2.loc[df2['DOW'] == value]
        df3 = df3.loc[df3['DOW'] == value]
        df4 = df4.loc[df4['DOW'] == value]
    else:
    ...
    if 'HKQuantityTypeIdentifierRestingHeartRate' not in df4.values:
        ...
        resting_heart_rate = str(round(df4[df4['@type'] == 'HKQuantityTypeIdentifierRestingHeartRate'].iloc[0]['@Value'],2))
    if 'HKQuantityTypeIdentifierWalkingHeartRateAverage' not in df4.values:
        walking_heart_rate = 'Not measured'
    else:
        walking_heart_rate = str(round(df4.loc[df4['@type'] == 'HKQuantityTypeIdentifierWalkingHeartRateAverage'].iloc[0]['@Value'], 2))
    ...
    if 'HKQuantityTypeIdentifierStepCount' not in df4.values:
        ...
        if group == 'D':
            ...
            step = str(round(df2.loc[df2['@type'] == 'HKQuantityTypeIdentifierStepCount'].iloc[0]['@Value'], 2)) \
                 + '|' + str(round(df3.loc[df3['@type'] == 'HKQuantityTypeIdentifierStepCount'].iloc[0]['@Value'], 2))
