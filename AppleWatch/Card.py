...
def card_update(df):
    ...
    if 'Resting Heart Rate' not in df.key.values:
        ...
        resting_hr = df[df['key'] == 'Resting Heart Rate']["Value"]

    if 'Walking Heart Rate Average' not in df.key.values:
        walking_hr = 'Not measured'
    else:
        walking_hr = df[df['key'] == 'Walking Heart Rate Average']["Value"]

    if 'Heart Rate' not in df.key.values:
        hr_mean = 'Not measured'
    else:
        hr_mean = df[df['key'] == 'Heart Rate']["Value"]

    if 'Step Count' not in df.key.values:
        step = 'Not measured'
    else:
        step = df[df['key'] == 'Step Count']["Value"]

    if 'Apple Exercise Time' not in df.key.values:
        exercise_minute = 'Not measured'
    else:
        exercise_minute = df[df['key'] == 'Apple Exercise Time']["Value"]

    if 'Active Energy Burned' not in df.key.values:
        activity_summary = 'Not measured'
    else:
        activity_summary = df[df['key'] == 'Active Energy Burned']["Value"]
